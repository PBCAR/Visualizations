...
# SELECT level of significance (alpha level)
sign_level <- 0.05
...
melted.corr.pval$pval_sign <- ifelse(melted.corr.pval$pval<sign_level,"Sign.","N.S.")
melted.corr.pval$pval_sign <- factor(melted.corr.pval$pval_sign, levels = c("Sign.","N.S."))

### OPTIONAL WRITE a .csv file with p-values of significance
# write.csv(melted.corr.pval,"heatmap.pvalues.csv", row.names = F)

##### ----------  MERGE p-values with correlation coefficients
#################################################################################################

melted.corr.pval <- melted.corr.pval[c("Var1","Var2","pval_sign")]

melted.corr <- merge(melted.corr.matrix, melted.corr.pval,
                     by = c("Var1","Var2"), all.x = T)

#################################################################################################
##### STEP 3: HEATMAP CREATION
#################################################################################################

### Significant correlation coefficients (based on 'sign_level') will be bold (N.S. will be grey)
...
heatmap.plot <- ggplot(data = melted.corr, aes(Var2, Var1, fill = value)) +
  ...
  theme_minimal() +
  coord_fixed() + 
  geom_text(aes(Var2, Var1, label = value, colour = pval_sign), size = 4, show.legend = F) +
  scale_colour_manual(values = c("#000000","#7A7A7A"))

heatmap.plot + ggtitle(label = hmap.title)  +
  theme(
    ...
    axis.text.x = element_text(face = "bold", vjust = 0.5, angle = 30,size = 10),
    axis.text.y = element_text(face = "bold", size = 10)) +
    guides(fill = guide_colorbar(barwidth = 7, barheight = 1,
                                 title.position = "top", title.hjust = 0.5))

#################################################################################################
##### STEP 4: NUMBER OF PAIRED OBS
#################################################################################################
